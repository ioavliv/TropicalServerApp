...
namespace TropicalServer.DAL
{
    public class ReportsDAL
    {
         ...
         */
        public DataSet GetUsersSetting_DAL(string key, string cId)
        {
            DataSet ds = new DataSet();
            ...
            try
            {
                ...
                {
                    connection.Open();
                    string query = "select * from master.dbo.tblTropicalUser where "+ key +" = '" + cId + "'";
        ...
        public DataSet GetOrders_DAL()
        {
            ...
            try
            {
                ...
                {
                    connection.Open();
                    string query = "select* from tblOrder o " +
                    "inner join tblCustomer c " +
                    "on o.OrderCustomerNumber = c.CustNumber " +
                    "where OrderTrackingNumber is not null"; 
                    SqlCommand command = new SqlCommand(query, connection);
                    command.CommandType = CommandType.Text;
                    command.CommandTimeout = 6000;
                    SqlDataAdapter adp = new SqlDataAdapter(command);
        ...
        public DataSet GetFilteredOrders_DAL(string condition)
        {
            DataSet ds = new DataSet();
